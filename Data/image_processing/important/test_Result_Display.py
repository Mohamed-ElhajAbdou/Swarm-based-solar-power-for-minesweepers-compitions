...
while cap.isOpened():
    ...
    Fitr = cv2.blur(result_frame, (1, 1))
    Fitr2 = cv2.blur(xeco, (1, 1))

    iddt1 = cv2.idct(Fitr)
    iddt2 = cv2.idct(Fitr2)
    iddt4 = cv2.idct(Desc)


    #iddt3 = cv2.idct(Fitr2)
    #cv2.imshow('res3 iddt3 LPF', iddt1)
    # cv2.imshow(' AFter the LOW PASS FIlter ', iddt1)
    # cv2.imshow(' Removing F(0,0)from the DCT Matrix ', iddt2)
    ####################################################################################################################################################################
...
#    res3 = cv2.bitwise_and(iddt3, iddt3, mask=mask)
    #cv2.imshow(' Removing F(0,0)res2 ', res2)
